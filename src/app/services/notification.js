...
async function sendNotification(content, player_ids, title,schtime) {
  try {
    ...
    notification.name = "Canada Halal";
...
module.exports = {
  notify: async (user,title,content,type) => {
    const player_ids = await findDevices(user);
    // console.log('player_ids====>', player_ids)
    const notObj = { for: user, description: content, title: title,type:type };
    console.log('notobj',notObj)
    await Notification.create(notObj);
    return sendNotification(content, player_ids, title);
  },
  schedulenotify: async (user,title,content,type,schtime) => {
    const player_ids = await findDevices(user);
    // console.log('player_ids====>', player_ids)
    const notObj = { for: user, description: content, title: title,type:type };
    console.log('notobj',notObj)
    await Notification.create(notObj);
    return sendNotification(content, player_ids, title,schtime);
  },
